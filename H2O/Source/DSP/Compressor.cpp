...
CCompressor::CCompressor(float sampleRate)
{
    ...
    LUT = new float[static_cast<size_t>(nLUT)];
    dLUT = new float[static_cast<size_t>(nLUT)];
...
//attack in ms
void CCompressor::setAttack(float value)
{
    attack = 5.f + value * 145.f;
    if (attack < 1.f)
        attack = 1.f;
    ga = (float)std::exp(-1 / (samplerate * attack / 1000.f));
}
...
void CCompressor::setRelease(float r)
{
    ...
    if (release < 1.f)
        release = 1.f;
    gr = (float)std::exp(-1 / (samplerate * release / 1000.f));
}
...
void CCompressor::setAmount(float value)
{
    this->a = value;
    amount = value * 0.995f;
    CalcLUT();
