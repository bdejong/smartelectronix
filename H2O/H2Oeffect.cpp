...
void H2Oeffect::setParameter(VstInt32 index, float value)
{
    ...
    case kRelease:
        ...
        break;
    case kAmount:
        fAmount = ap->fAmount = value;
        comp->setAmount(value);
        break;
    case kPostAmp:
        fPostAmp = ap->fPostAmp = value;
        comp->setPostamp(value);
        break;
    case kPreAmp:
        fPreAmp = ap->fPreAmp = value;
        comp->setPreamp(value);
        break;
    case kSaturate:
        fSaturate = ap->fSaturate = value;
        comp->setSaturate(value);
        break;
    }
}
...
void H2Oeffect::getParameterDisplay(VstInt32 index, char* text)
{
    switch (index) {
    case kAttack:
        float2string2(comp->getAttack(), text);
        break;
    case kRelease:
        float2string2(comp->getRelease(), text);
        break;
    case kAmount:
        float2string2(comp->getAmount(), text);
        break;
    case kPostAmp:
        dB2string2(comp->getPostamp(), text);
        break;
    case kPreAmp:
        dB2string2(comp->getPreamp(), text);
        break;
    case kSaturate:
        if (comp->getSaturate())
            strcpy(text, "on");
