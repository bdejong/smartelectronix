                                             ...
                                             juce::Slider& slider)
{
    ...
    juce::Path trackPath;
    trackPath.addCentredArc(centreX, centreY, radius, radius, 0.0f,
                            rotaryStartAngle, rotaryEndAngle, true);
    g.strokePath(trackPath, juce::PathStrokeType(trackThickness,
    ...
    if (sliderPos > 0.0f)
    {
        ...
        juce::Path fillPath;
        fillPath.addCentredArc(centreX, centreY, radius, radius, 0.0f,
                               rotaryStartAngle, angle, true);
        g.strokePath(fillPath, juce::PathStrokeType(trackThickness,
                     juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
    }
...
void SupaTriggaLookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& button,
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), apvts(vts)
{
    ...
    rearrangeAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::PROB_REARRANGE_ID, rearrangeKnob);

    setupKnob(slicesKnob, Section::Global);
    slicesAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::GRANULARITY_ID, slicesKnob);

    setupKnob(silenceKnob, Section::Global);
    silenceAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::PROB_SILENCE_ID, silenceKnob);

    // Brake section
    setupKnob(brakeProbKnob, Section::Speed);
    brakeProbAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::PROB_SPEED_ID, brakeProbKnob);

    setupKnob(brakeTimeKnob, Section::Speed);
    brakeTimeAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::SPEED_ID, brakeTimeKnob);

    setupToggle(brakeInstantToggle, Section::Speed);
    brakeInstantAttach = std::make_unique<ButtonAttachment>(
        apvts, SupaTriggaProcessor::INSTANT_SPEED_ID, brakeInstantToggle);

    // Reverse section
    setupKnob(reverseProbKnob, Section::Reverse);
    reverseProbAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::PROB_REVERSE_ID, reverseProbKnob);

    setupToggle(reverseInstantToggle, Section::Reverse);
    reverseInstantAttach = std::make_unique<ButtonAttachment>(
        apvts, SupaTriggaProcessor::INSTANT_REVERSE_ID, reverseInstantToggle);

    // Repeat section
    setupKnob(repeatProbKnob, Section::Repeat);
    repeatProbAttach = std::make_unique<SliderAttachment>(
        apvts, SupaTriggaProcessor::PROB_REPEAT_ID, repeatProbKnob);

    setupToggle(repeatInstantToggle, Section::Repeat);
    repeatInstantAttach = std::make_unique<ButtonAttachment>(
        apvts, SupaTriggaProcessor::INSTANT_REPEAT_ID, repeatInstantToggle);
...
SupaTriggaEditor::~SupaTriggaEditor()
{
...
void SupaTriggaEditor::paint(juce::Graphics& g)
{
    ...
    g.drawText("BY SMARTELECTRONIX", getWidth() - 200, 0, 180, 56,
    ...
    g.drawLine((float)midX, (float)(gridBounds.getY() + 16),
               (float)midX, (float)(gridBounds.getBottom() - 16), 1.0f);
    ...
    auto drawLabel = [&](const juce::String& text, juce::Component& comp)
    {
        ...
        g.drawText(text, bounds.getX() - 20, bounds.getY() - 28,
                   bounds.getWidth() + 40, 20, juce::Justification::centredBottom, false);
    };
