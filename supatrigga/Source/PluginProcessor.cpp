...
SupaTriggaProcessor::createParameterLayout() {
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      ...
      juce::NormalisableRange<float>(0.0f, 1.0f, 0.001f), 0.3f, juce::String(),
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      ...
      juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.15f, juce::String(),
      juce::AudioProcessorParameter::genericParameter, probToString, nullptr));

  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      juce::ParameterID(PROB_SPEED_ID, 1), "Slow Prob",
      juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.05f, juce::String(),
      juce::AudioProcessorParameter::genericParameter, probToString, nullptr));

  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      juce::ParameterID(PROB_REARRANGE_ID, 1), "Rearrange Prob",
      juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.95f, juce::String(),
      juce::AudioProcessorParameter::genericParameter, probToString, nullptr));
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      ...
      juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.4f, juce::String(),
      juce::AudioProcessorParameter::genericParameter, probToString, nullptr));
...
const juce::String SupaTriggaProcessor::getProgramName(int /*index*/) {
  return {};
}
void SupaTriggaProcessor::changeProgramName(int /*index*/,
                                            const juce::String & /*newName*/) {}

void SupaTriggaProcessor::prepareToPlay(double sampleRate,
                                        int /*samplesPerBlock*/) {
...
bool SupaTriggaProcessor::isBusesLayoutSupported(
...
void SupaTriggaProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  if (!isPlaying || !bpmOpt.hasValue() || !timeSigOpt.hasValue() ||
      !ppqPosOpt.hasValue()) {
  ...
  double samplesInMeasureDouble = std::ceil(
      numSamplesInBeat * timeSig.numerator / (timeSig.denominator / 4.0));
  unsigned long samplesInMeasure =
      static_cast<unsigned long>(samplesInMeasureDouble);
  ...
  double numSamplesToNextBar =
      (distanceToNextBarPPQ * currentSampleRate * 60.0) / tempo;
  if (numSamplesToNextBar < 0.0)
    numSamplesToNextBar = 0.0;

  unsigned long samplesToNextBar =
      static_cast<unsigned long>(std::ceil(numSamplesToNextBar));

  // If playback changed, recalculate position
  if (playbackChanged) {
    positionInMeasure = static_cast<unsigned long>(
        std::floor(samplesInMeasureDouble - numSamplesToNextBar));
  ...
  unsigned long granularityTmp =
      static_cast<unsigned long>(granularityRaw * (BITSLIDES + 0.5f));
  unsigned long granularityMaskTmp = 0xffffffff << (BITSLIDES - granularityTmp);

  float speedDiff = std::exp(std::log(0.01f) / (((1.0f - speedRaw) + 0.01f) *
                                                currentSampleRate * 4.0f));
  ...
  for (int i = 0; i < sampleFrames; i++) {
    ...
    unsigned long sliceIndex =
        ((positionInMeasure * MAXSLIDES) / samplesInMeasure) & granularityMask;

    if (instantRepeat && sliceIndex != 0)
      sequencer[sliceIndex].offset =
          (sequencer[(sliceIndex - 1) & granularityMaskTmp].offset +
           (sliceIndex - ((sliceIndex - 1) & granularityMaskTmp))) &
          granularityMask;

    unsigned long displacement = sequencer[sliceIndex].offset & granularityMask;

    if (granularityMaskTmp != granularityMask) {
      unsigned long sliceIndexTmp =
          ((positionInMeasure * MAXSLIDES) / samplesInMeasure) &
          granularityMaskTmp;
    ...
    // Gain calculation
    {
      unsigned long sliceIndexFar =
          ((((positionInMeasure + FADETIME) % samplesInMeasure) * MAXSLIDES) /
           samplesInMeasure) &
          granularityMask;
      unsigned long displacementFar =
          sequencer[sliceIndexFar].offset & granularityMask;
    ...
    if ((sequencer[sliceIndex].reverse || instantReverse) &&
        displacement != 0) {
      if (!(sequencer[sliceIndex].stop || instantSlow)) {
        ...
        unsigned long difference =
            (displacement * samplesInMeasure) / MAXSLIDES;
        unsigned long bufferIndex =
            difference <= sliceEnd ? sliceEnd - difference : 0;
        ...
        if (first) {
          ...
          unsigned long sliceStart =
              (positionInMeasure / sliceSize) * sliceSize;
          unsigned long sliceDiff = positionInMeasure - sliceStart;
          unsigned long sliceEnd = (sliceStart + sliceSize) - sliceDiff;
          unsigned long difference =
              (displacement * samplesInMeasure) / MAXSLIDES;
      ...
      if (!(sequencer[sliceIndex].stop || instantSlow) || displacement == 0) {
        unsigned long difference =
            (displacement * samplesInMeasure) / MAXSLIDES;
        unsigned long bufferIndex =
            positionInMeasure > difference ? positionInMeasure - difference : 0;
        ...
        if (first) {
          position =
              static_cast<float>(positionInMeasure -
                                 (displacement * samplesInMeasure) / MAXSLIDES);
          speed = 1.0f / speedDiff;
          first = false;
        }
  ...
  for (unsigned long i = 0; i < MAXSLIDES; i++) {
    if ((std::rand() % 100) < static_cast<int>(probRearrangeRaw * 101.0f)) {
      if ((std::rand() % 100) < static_cast<int>(probRepeatRaw * 101.0f)) {
        if (i != 0)
          sequencer[i].offset =
              (sequencer[(i - 1) & granularityMaskTmp].offset +
               (i - ((i - 1) & granularityMaskTmp))) &
              granularityMask;
        else
          sequencer[i].offset = 0;
      } else {
        sequencer[i].offset =
            (i * static_cast<unsigned long>(std::rand() % MAXSLIDES)) /
            MAXSLIDES;
      }
    } else {
      sequencer[i].offset = 0;
    ...
    if ((std::rand() % 100) < static_cast<int>(probReverseRaw * 100.0f) &&
        sequencer[i].offset > 0)
...
void SupaTriggaProcessor::setStateInformation(const void *data,
                                              int sizeInBytes) {
