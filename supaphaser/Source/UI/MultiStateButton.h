...
{
public:
    MultiStateButton(const juce::Image& filmStrip, int numStates)
        : filmStrip(filmStrip), numStates(numStates)
    {
        frameHeight = filmStrip.getHeight() / numStates;
    ...
    void setValue(float v)
    {
        int newState = juce::jlimit(0, numStates - 1,
            static_cast<int>(v * static_cast<float>(numStates) + 0.001f));
...
private:
    float getValueForState() const
    {
        ...
        return static_cast<float>(currentState) / static_cast<float>(numStates - 1);
    }
...
{
public:
    MultiStateButtonAttachment(juce::AudioProcessorValueTreeState& apvts,
                               const juce::String& parameterID,
                               MultiStateButton& button)
        : apvts(apvts), paramId(parameterID), button(button)
    {
